...
countMatrix <-    read.delim(sub(".gff.gz", ".o2o_aln.matrix.txt", alnFilePath), sep = '\t', comment.char = '#',  header = TRUE, row.names = 1, check.names = FALSE) |> as.matrix()
...
report$JC69 <- JC69_distance(countMatrix)
report$K80  <-  K80_distance(countMatrix)
report$T92  <-  T92_distance(countMatrix)

# Should not need it, but...

NAifNULL <- \(x) if(is.null(x)) NA else x

# Pipeline v.2.3
report$F81                     <- F81_distance(countMatrix)
...
report$P                       <- P_distance(countMatrix)
report$GBreaksPipelineVersion  <- "0.17.0.1"

report$GCequi                  <- GCequilibrium(countMatrix)
report$GCpres                  <- GCpressure(countMatrix)
report$GCprop                  <- GCproportion(countMatrix)
report$gapProp                 <- gapProportion(countMatrix)
report$P_error                 <- P_error(countMatrix)
report$GBreaksPipelineVersion  <- "0.19.0.1"


report$TN93                    <-TN93_distance(countMatrix)
report$HKY85                   <-HKY85_distance(countMatrix)
report$LogDet                  <-logDet_distance(countMatrix)
